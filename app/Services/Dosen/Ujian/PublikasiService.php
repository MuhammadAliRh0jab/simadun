...
{
    public function store($request, $publikasiId)
    {
        $publikasi = PendaftaranUjian::findOrFail($publikasiId);
        JadwalUjian::create([
            'pendaftaran_ujian_id' => $publikasiId,
        ...
        Alert::success('Berhasil', 'Status publikasi berhasil diubah');
        return redirect()->route('dosen.ujian.publikasi');
    }

    public function show($id)
    {
        $publikasi = PendaftaranUjian::findOrFail($id);
        $mahasiswa = Mahasiswa::where('nim', $publikasi->nim)->first();
        ...
        $publikasi_jurnal = PublikasiJurnal::where('nim', $mahasiswa->nim)->get();
        $publikasi_conference = PublikasiConference::where('nim', $mahasiswa->nim)->get();
        ...
        if ($jadwal_ujian) {
            ...
            // Dosens komentars
            $publikasi->komentar1 = KomentarUjian::select('komentar')->where('role', 'dosen')->where('pendaftaran_ujian_id', $id)->where('dosen_id', $jadwal_ujian->penguji1_id)->value('komentar');
            $publikasi->komentar2 = KomentarUjian::select('komentar')->where('role', 'dosen')->where('pendaftaran_ujian_id', $id)->where('dosen_id', $jadwal_ujian->penguji2_id)->value('komentar');
            $publikasi->komentar3 = $jadwal_ujian->penguji3_id ? KomentarUjian::select('komentar')->where('role', 'dosen')->where('pendaftaran_ujian_id', $id)->where('dosen_id', $jadwal_ujian->penguji3_id)->value('komentar') : null;
            $publikasi->komentar4 = KomentarUjian::select('komentar')->where('role', 'dosen')->where('pendaftaran_ujian_id', $id)->where('dosen_id', $mahasiswa->promotor_id)->value('komentar');
            $publikasi->komentar5 = KomentarUjian::select('komentar')->where('role', 'dosen')->where('pendaftaran_ujian_id', $id)->where('dosen_id', $mahasiswa->co_promotor1_id)->value('komentar');
            $publikasi->komentar6 = KomentarUjian::select('komentar')->where('role', 'kaprodi')->where('pendaftaran_ujian_id', $id)->value('komentar');
            $publikasi->komentar7 = $mahasiswa->co_promotor2_id ? KomentarUjian::select('komentar')->where('role', 'dosen')->where('pendaftaran_ujian_id', $id)->where('dosen_id', $mahasiswa->co_promotor2_id)->value('komentar') : null;
            ...
            $publikasi->is_penilaian_done = $publikasi->nilai_penguji1 != null &&
                                            ...
                                            ($mahasiswa->co_promotor2_id == null || $publikasi->nilai_co_promotor2 != null) &&
        ...
        return view('pages.dosen.ujian.publikasi.show', compact('publikasi', 'mahasiswa', 'jadwal_ujian', 'publikasi_jurnal', 'publikasi_conference'));
    }
    ...
    public function finalisasi($request)
    {
        $request->validate(
            [
                ...
                'publikasi_id' => 'required|numeric',
                'role' => 'required|string|in:kaprodi',
                'status' => 'required|string|in:layak,revisi',
                'jenis_ujian' => 'required|string|in:publikasi'
            ],
            [
                'mahasiswa_id.required' => 'Tidak valid',
                'mahasiswa_id.numeric' => 'Tidak valid',
                'publikasi_id.required' => 'Ujian tidak valid',
                'publikasi_id.numeric' => 'Ujian tidak valid',
        ...
        $publikasi = PendaftaranUjian::findOrFail($request->publikasi_id);
        $mahasiswa = Mahasiswa::findOrFail($request->mahasiswa_id);
        $publikasi->status = $request->status == 'layak' ? '6' : '4';
        $publikasi->save();
        $mahasiswa->progress = $request->status == 'layak' ? '5' : '3';
        $mahasiswa->save();
