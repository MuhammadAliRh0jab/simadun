...
<div class="col-lg-12 mb-4 h-100" id="ujian_proposan">
  <div class="card mb-3">
    <div class="card-header pb-4 d-flex justify-content-between mb-lg-n4">
      <div class="card-title mb-3">
        <h2 class="mb-2">Ujian Seminar Hasil</h2>
        <small class="text-muted">Proses Ujian Seminar Hasil. Tahap ini terdiri dari 4 tahapan utama yaitu
  ...
  <div class="card d-none" data-step="0">
    ...
    <div class="card-body">
      <div class="row">
        <div class="col-lg-12">
          <form action="{{ route('ujian.semhas.store') }}" method="post" id="pendaftaran">
            @csrf
            {{-- nama nim --}}
            ...
            <div class="form-group mb-3">
              ...
              <select class="form-select @error('usulan_penguji') is-invalid @enderror" multiple="multiple"
                ...
                @if ($semhas)
                  <option value="{{ $semhas->penguji1_id ?? '' }}" selected>
                    {{ $semhas->usulan_penguji1 ?? '' }}
                  </option>
                  <option value="{{ $semhas->penguji2_id ?? '' }}" selected>
                    {{ $semhas->usulan_penguji2 ?? '' }}
                  </option>
                  @if ($semhas->penguji3_id)
                    <option value="{{ $semhas->penguji3_id }}" selected>
            ...
            <div class="form-group mb-3">
              ...
              <input type="url" class="form-control naskah @error('naskah') is-invalid @enderror" name="naskah"
                value="{{ old('naskah', $semhas->file ?? '') }}" placeholder="Masukkan link GDrive naskah" required>
              @error('naskah')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
          ...
          <div class="row mb-3">
            ...
            <div class="col-sm-7 col-md-9">
              <p class="mb-0" id="penguji">
                @if ($semhas)
                  {{ $semhas->usulan_penguji1 ?? '' }}
                  @if ($semhas->usulan_penguji2)
                    , {{ $semhas->usulan_penguji2 }}
                  @endif
                  @if ($semhas->usulan_penguji3)
                    , {{ $semhas->usulan_penguji3 }}
                  @endif
                @else
                  Belum ada penguji
        ...
        @if ($semhas && $semhas->status == 2)
          <div class="col-12">
            ...
            <div class="row mb-3 justify-content-center align-items-center">
              ...
              <div class="col-sm-7 col-md-5">
                <p class="mb-0" id="penguji1">{{ $semhas->usulan_penguji1 ?? '' }}</p>
              </div>
            </div>
            <div class="row mb-3 justify-content-center align-items-center">
              <p class="col-sm-5 col-md-3 form-label mb-0"><b>Usulan Penguji 2</b></p>
              <div class="col-sm-7 col-md-5">
                <p class="mb-0" id="penguji2">{{ $semhas->usulan_penguji2 ?? '' }}</p>
              </div>
            </div>
            @if ($semhas && $semhas->usulan_penguji3)
              <div class="row mb-3 justify-content-center align-items-center">
                <p class="col-sm-5 col-md-3 form-label mb-0"><b>Usulan Penguji 3</b></p>
                <div class="col-sm-7 col-md-5">
                  <p class="mb-0" id="penguji3">{{ $semhas->usulan_penguji3 ?? '' }}</p>
                </div>
              </div>
            @endif
            <div class="row mb-3 justify-content-center align-items-center">
          ...
          <div class="col-12 col-md-8">
            <div class="row mb-4">
              ...
              <div class="">
                ...
                @else
                  <span class="badge bg-label-danger m  m-1"><i class="ti ti-circle-x"></i> Co-Promotor 1</span>
                @endif
                @if ($semhas->nilai_co_promotor2 == 100)
                  <span class="badge bg-label-success m-1"><i class="ti ti-circle-check"></i> Co-Promotor 2</span>
                @elseif($semhas->nilai_co_promotor2 === null)
                @else
            ...
            <div class="row mb-4">
              ...
              <div class="">
                ...
                @endif
                @if ($semhas->komentar3_penguji)
                  <button class="btn btn-light m-1 komentar" data-bs-toggle="modal" data-bs-target="#modalComment"
                    onclick='showk({!! json_encode($semhas->komentar3_penguji) !!}, 3)'>
                    <i class="ti ti-message me-2" id="penguji3"></i> Penguji 3
                  </button>
                @endif
                @if ($semhas->komentar3)
                  <button class="btn btn-light m-1 komentar" data-bs-toggle="modal" data-bs-target="#modalComment"
                    onclick='showk({!! json_encode($semhas->komentar3) !!}, "Promotor")'>
                    <i class="ti ti-message me-2" id="promotor"></i> Promotor
                  </button>
                @endif
                @if ($semhas->komentar4)
                  <button class="btn btn-light m-1 komentar" data-bs-toggle="modal" data-bs-target="#modalComment"
                    onclick='showk({!! json_encode($semhas->komentar4) !!}, "Co-Promotor 1")'>
                    <i class="ti ti-message me-2" id="co_promotor1"></i> Co-Promotor 1
                  </button>
                @endif
                @if ($semhas->komentar5)
                  <button class="btn btn-light m-1 komentar" data-bs-toggle="modal" data-bs-target="#modalComment"
                    onclick='showk({!! json_encode($semhas->komentar5) !!}, "Co-Promotor 2")'>
                    <i class="ti ti-message me-2" id="co_promotor2"></i> Co-Promotor 2
                  </button>
                @endif
                @if ($semhas->komentar6)
                  <button class="btn btn-light m-1 komentar" data-bs-toggle="modal" data-bs-target="#modalComment"
                    onclick='showk({!! json_encode($semhas->komentar6) !!}, "Kaprodi")'>
            ...
            @if ($semhas->status == 4)
              <form action="{{ route('ujian.semhas.revisi') }}" method="post" id="revisi">
                @csrf
                @method('patch')
                <h5 class="mt-3 mb-1"><b>Form Revisi Naskah</b></h5>
                <input type="hidden" name="id_pendaftaran" value="{{ $semhas->id }}">
                <div class="form-group mb-3">
            ...
            @elseif ($semhas->status == 6)
              ...
              <div class="form-group mb-3 text-center">
                <a href="{{ route('ujian.publikasi') }}" class="btn btn-primary btn-next">Lanjutkan
