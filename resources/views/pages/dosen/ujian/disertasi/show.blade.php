      ...
      <div class="col-lg-8">
        <div class="card">
          <div class="card-header">
            <h4 class="card-header-title">Data Pengajuan Ujian Seminar Hasil</h4>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="row mb-3">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-7 col-md-8">
                  <p class="mb-0">{{ $disertasi->usulan_penguji1 }}</p>
                </div>
              </div>
              <div class="row mb-3">
                <p class="col-sm-5 col-md-4 form-label mb-0"><b>Usulan Penguji 2</b></p>
                <div class="col-sm-7 col-md-8">
                  <p class="mb-0">{{ $disertasi->usulan_penguji2 }}</p>
                </div>
              </div>
              @if ($disertasi->usulan_penguji3 != '-')
                <div class="row mb-3">
                  ...
                  <div class="col-sm-7 col-md-8">
                    <p class="mb-0">{{ $disertasi->usulan_penguji3 }}</p>
                  </div>
                </div>
              @endif
              <div class="row mb-3">
                <p class="col-sm-5 col-md-4 form-label mb-0"><b>Berkas Naskah @if ($disertasi->status == 5) (Revisi) @endif</b></p>
                <div class="col-sm-7 col-md-8">
                  <p class="mb-0">
        ...
        @if (
            $disertasi->nilai_penguji1 ||
            $disertasi->nilai_penguji2 ||
            $disertasi->nilai_penguji3 ||
            $disertasi->nilai_promotor ||
            $disertasi->nilai_co_promotor1 ||
            $disertasi->nilai_co_promotor2 ||
            $disertasi->nilai_kaprodi
        )
          <div class="card mt-3">
            ...
            <div class="card-body">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-7 col-md-8">
                  <p class="mb-0">
                    @if ($disertasi->nilai_co_promotor1)
              ...
              @if ($mahasiswa->co_promotor2_id)
                <div class="row mb-3">
                  ...
                  <div class="col-sm-7 col-md-8">
                    <p class="mb-0">
                      @if ($disertasi->nilai_co_promotor2)
        ...
        @if ($disertasi->komentar1 || $disertasi->komentar2 || $disertasi->komentar3 || $disertasi->komentar4 || $disertasi->komentar5 || $disertasi->komentar6 || $disertasi->komentar7)
          <div class="card mt-3">
            <div class="card-header mb-0">
              ...
              <div class="row mb-3">
                <div class="col">
                  ...
                  @if ($disertasi->komentar4)
                    <button class="btn btn-light m-1" data-bs-toggle="modal" data-bs-target="#modalComment"
                      onclick='showk({!! json_encode($disertasi->komentar4) !!}, 4)'>
                      <i class="ti ti-message me-2"></i> Promotor
                    </button>
                  @endif
                  @if ($disertasi->komentar5)
                    <button class="btn btn-light m-1" data-bs-toggle="modal" data-bs-target="#modalComment"
                      onclick='showk({!! json_encode($disertasi->komentar5) !!}, 5)'>
                      <i class="ti ti-message me-2"></i> Co-Promotor 1
                    </button>
                  @endif
                  @if ($disertasi->komentar7)
                    <button class="btn btn-light m-1" data-bs-toggle="modal" data-bs-target="#modalComment"
                      onclick='showk({!! json_encode($disertasi->komentar7) !!}, 7)'>
                      <i class="ti ti-message me-2"></i> Co-Promotor 2
                    </button>
                  @endif
                  @if ($disertasi->komentar6)
                    <button class="btn btn-light m-1" data-bs-toggle="modal" data-bs-target="#modalComment"
                      onclick='showk({!! json_encode($disertasi->komentar6) !!}, 6)'>
        ...
        @if (!$disertasi->nilai_kaprodi && ($jadwal_ujian && $jadwal_ujian->penilaian))
          <div class="card mt-3">
            <div class="card-header mb-0">
          ...
          <div class="card mt-3">
            ...
            <div class="card-body">
              <div class="row mb-3">
                ...
                <div class="col-sm-7 col-md-8">
                  <div class="mb-3">
                    @if ($disertasi->jenis_ujian != 'tertutup' && $disertasi->nilai_kaprodi == 100)
        ...
        @if (!$jadwal_ujian)
          <div class="card mt-3">
            ...
            <div class="card-body">
              <form action="{{ route('dosen.ujian.disertasi.store', $disertasi->id) }}" method="POST" id="penjadwalan">
                @csrf
                <div class="row mb-3">
        ...
        @endif
        @if ($disertasi->is_penilaian_done && $disertasi->status < 6)
          <div class="card mt-3">
            <div class="card-header mb-0">
            ...
            <div class="card-body">
              <div class="row mb-3">
                <div class="col d-flex justify-content-center gap-2 my-3 w-100">
                  <button type="button" class="w-50 btn btn-warning"
                    @if ($disertasi->status != 4) onclick="finalisasi('revisi', event)" @else disabled @endif>
                    <i class="ti ti-circle-x me-2"></i> <b>Revisi</b>
                  </button>
                  <button type="button" class="w-50 btn btn-success"
                    @if ($disertasi->status != 4) onclick="finalisasi('layak', event)" @else disabled @endif>
                    <i class="ti ti-circle-check me-2"></i> <b>Layak</b>
                  </button>
                </div>
              </div>
              ...
              @else
                <div class="mt-3 alert alert-info d-flex align-items-center" role="alert">
                  <i class="ti ti-exclamation-circle me-1 fw-bold"></i>
                  <div><strong>Perhatian!</strong> Tombol Finalisasi hanya akan muncul jika status pengajuan seminar hasil adalah <b>Revisi Diajukan</b>. Harap menunggu hingga mahasiswa mengajukan ulang naskah.</div>
                </div>
              @endif
            </div>
          </div>
...
@section('scripts')
      ...
      window.finalisasi = function(status, e) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axios.post("{{ route('dosen.ujian.disertasi.finalisasi') }}", {
              _token: "{{ csrf_token() }}",
              _method: "PATCH",
              mahasiswa_id: "{{ $mahasiswa->id }}",
              disertasi_id: "{{ $disertasi->id }}",
              role: "kaprodi",
              jenis_ujian: "{{ $disertasi->jenis_ujian }}",
              status: status,
            }).then((response) => {
              Swal.fire({
                title: 'Berhasil!',
                ...
                if (result.isConfirmed) {
                  window.location.href = "{{ route('dosen.ujian.disertasi') }}";
                }
              });
            }).catch((error) => {
  ...
  <script>
    $(document).ready(function() {
      ...
      @if ($disertasi->komentar6)
        const komentar = {!! $disertasi->komentar6 !!};
        if (komentar) {
          snowEditor.disable();
  ...
  <script>
    $('#penjadwalan').submit(function(e) {
      ...
      Swal.fire({
        title: 'Apakah anda yakin?',
        text: "Anda akan menjadwalkan ujian disertasi mahasiswa ini!",
        icon: 'warning',
        customClass: { confirmButton: 'btn btn-success', cancelButton: 'btn btn-danger' },
      }).then((result) => {
